...
const DOC_PAGES: DocPage[] = [
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "需要收藏和复查生成图时阅读“画廊”。",
              "需要连续改图时阅读“文/图生图概览”“当前作品和参考图”“生成设置”和“任务与结果”。",
        ...
        blocks: [
          {
            ...
            rows: [
              ...
              ["桌面底部历史记录", "按分支展示历史结果。点击已完成图片会把它选为当前结果，并作为下一轮上下文。"],
              ...
              ["移动端右侧历史抽屉", "以窄抽屉展示分支和候选。点已完成图片会选为当前结果和下一轮上下文，点占位会查看该候选状态。"],
  ...
  },
  {
    ...
    title: "当前作品和参考图",
    description: "说明文/图生图里当前作品、会话参考图的关系，以及单轮图片上下文数量限制。",
    category: "文/图生图",
    icon: Sparkles,
    sections: [
      {
        ...
        blocks: [
          {
            type: "paragraph",
            text: "文/图生图会自动把当前作品带入下一轮；点击历史候选即可切换创作分支。固定参考图用于补充风格、材质、姿态、背景等上下文。",
          },
          {
            type: "steps",
            items: [
              "第一轮没有历史图时，直接在“画面描述”里写想生成的画面。",
              "生成完成后，当前结果会自动成为下一轮上下文；点击底部历史记录或移动端历史抽屉中的候选即可切换分支。",
              "如需更多视觉参考，在右侧“会话参考图”上传图片；移动端在底部生成面板的生成设置标签页操作。",
              "固定参考图后再提交生成。系统会把当前作品和固定参考图一起作为本轮上下文。",
            ],
          },
        ],
      },
        ...
        blocks: [
          {
            type: "callout",
            title: "单轮最多 6 张",
            text: "单轮最多选择 6 张图片上下文，这个数量包含当前作品和固定参考图。如果当前作品存在，最多还能再使用 5 张参考图。",
          },
          {
            type: "list",
            items: [
              "想保留主体角度时，点击对应历史结果切换当前作品。",
...
const HELP_DOC_JA_TRANSLATIONS: Record<string, string> = {
  ...
  "需要连续改图时阅读“文/图生图概览”“当前作品和参考图”“生成设置”和“任务与结果”。":
    "継続的に画像を編集したい場合は「画像生成チャット概要」「現在の作品と参考画像」「生成設定」「タスクと結果」を読みます。",
  "按分支展示历史结果。点击已完成图片会把它选为当前结果，并作为下一轮上下文。":
    "履歴結果をブランチ別に表示します。完了画像をクリックすると現在結果として選択され、次ラウンドのコンテキストになります。",
  "以窄抽屉展示分支和候选。点已完成图片会选为当前结果和下一轮上下文，点占位会查看该候选状态。":
    "細いドロワーでブランチと候補を表示します。完了画像をタップすると現在結果と次ラウンドのコンテキストとして選択され、プレースホルダーをタップすると候補状態を確認できます。",
  "当前作品和参考图": "現在の作品と参考画像",
  "说明文/图生图里当前作品、会话参考图的关系，以及单轮图片上下文数量限制。":
    "画像生成チャットにおける現在の作品とセッション参考画像の関係、および1ラウンドの画像コンテキスト数制限を説明します。",
  "当前作品和参考图的关系": "現在の作品と参考画像の関係",
  "文/图生图会自动把当前作品带入下一轮；点击历史候选即可切换创作分支。固定参考图用于补充风格、材质、姿态、背景等上下文。":
    "画像生成チャットは現在の作品を自動的に次ラウンドへ引き継ぎます。履歴候補をクリックすると創作ブランチを切り替えられます。固定参考画像はスタイル、素材、ポーズ、背景などを補足します。",
  "生成完成后，当前结果会自动成为下一轮上下文；点击底部历史记录或移动端历史抽屉中的候选即可切换分支。":
    "生成後、現在の結果が自動的に次ラウンドのコンテキストになります。下部履歴またはモバイル履歴ドロワーの候補をクリックするとブランチを切り替えられます。",
  "固定参考图后再提交生成。系统会把当前作品和固定参考图一起作为本轮上下文。":
    "固定参考画像を選んでから生成を送信します。システムは現在の作品と固定参考画像をこのラウンドのコンテキストとして使用します。",
  "单轮最多选择 6 张图片上下文，这个数量包含当前作品和固定参考图。如果当前作品存在，最多还能再使用 5 张参考图。":
    "1ラウンドで選択できる画像コンテキストは最大6枚です。現在の作品と固定参考画像を含み、現在の作品がある場合は参考画像を最大5枚追加できます。",
  "想保留主体角度时，点击对应历史结果切换当前作品。": "主体の角度を保ちたい場合は、対応する履歴結果をクリックして現在の作品に切り替えます。",
};
...
const HELP_DOC_VI_TRANSLATIONS: Record<string, string> = {
  ...
  "需要连续改图时阅读“文/图生图概览”“当前作品和参考图”“生成设置”和“任务与结果”。": "Khi cần chỉnh sửa ảnh liên tục, hãy đọc \"Tổng quan về tạo ảnh\", \"Tác phẩm hiện tại và ảnh tham khảo\", \"Cài đặt tạo ảnh\" và \"Tác vụ và kết quả\".",
  "按分支展示历史结果。点击已完成图片会把它选为当前结果，并作为下一轮上下文。": "Hiển thị kết quả lịch sử theo nhánh. Nhấp vào ảnh đã hoàn thành để chọn làm kết quả hiện tại và bối cảnh cho lượt tiếp theo.",
  "以窄抽屉展示分支和候选。点已完成图片会选为当前结果和下一轮上下文，点占位会查看该候选状态。": "Hiển thị các nhánh và ứng viên trong ngăn kéo hẹp. Nhấp vào ảnh đã hoàn thành để chọn làm kết quả hiện tại và bối cảnh lượt tiếp theo; nhấp vào phần giữ chỗ để xem trạng thái ứng viên.",
  "当前作品和参考图": "Tác phẩm hiện tại và ảnh tham khảo",
  "说明文/图生图里当前作品、会话参考图的关系，以及单轮图片上下文数量限制。": "Giải thích mối quan hệ giữa tác phẩm hiện tại, ảnh tham khảo của phiên và giới hạn bối cảnh hình ảnh trong mỗi lượt.",
  "当前作品和参考图的关系": "Mối quan hệ giữa tác phẩm hiện tại và ảnh tham khảo",
  "文/图生图会自动把当前作品带入下一轮；点击历史候选即可切换创作分支。固定参考图用于补充风格、材质、姿态、背景等上下文。": "Tạo ảnh sẽ tự động đưa tác phẩm hiện tại vào lượt tiếp theo; nhấp vào ứng viên lịch sử để chuyển nhánh sáng tạo. Ảnh tham khảo cố định bổ sung phong cách, chất liệu, tư thế, nền và các bối cảnh khác.",
  "生成完成后，当前结果会自动成为下一轮上下文；点击底部历史记录或移动端历史抽屉中的候选即可切换分支。": "Sau khi tạo xong, kết quả hiện tại tự động trở thành bối cảnh cho lượt tiếp theo; nhấp vào ứng viên trong lịch sử phía dưới hoặc ngăn kéo lịch sử trên di động để chuyển nhánh.",
  "固定参考图后再提交生成。系统会把当前作品和固定参考图一起作为本轮上下文。": "Cố định ảnh tham khảo trước khi gửi tạo. Hệ thống sử dụng tác phẩm hiện tại và ảnh tham khảo cố định làm bối cảnh của lượt này.",
  "单轮最多选择 6 张图片上下文，这个数量包含当前作品和固定参考图。如果当前作品存在，最多还能再使用 5 张参考图。": "Mỗi lượt có tối đa 6 ảnh bối cảnh, bao gồm tác phẩm hiện tại và ảnh tham khảo cố định. Khi có tác phẩm hiện tại, có thể dùng thêm tối đa 5 ảnh tham khảo.",
  "想保留主体角度时，点击对应历史结果切换当前作品。": "Khi muốn giữ góc của chủ thể, hãy nhấp vào kết quả lịch sử tương ứng để chuyển tác phẩm hiện tại.",
};
